...
sh -n "$ROOT/include/run.sh" "$ROOT/include/bin/ap_start.sh" \
    "$ROOT/include/bin/heartbeat.sh" "$MODE" "$MANAGER" "$PORTAL_START" "$CGI"
...
printf '%s' "$body" |
    ...
    WIFI_REBOOT_COMMAND=/bin/true WIFI_REBOOT_DELAY=0 "$CGI" >"$TEST_DIR/response"
...
printf '%s' "$cancel" |
    ...
    WIFI_REBOOT_COMMAND=/bin/true WIFI_REBOOT_DELAY=0 "$CGI" >"$TEST_DIR/cancel-response"
